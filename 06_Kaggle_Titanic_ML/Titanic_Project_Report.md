...
**Analyst:** Mintay Misgano | **Year:** 2022 | **Tools:** Python, scikit-learn | **Best Kaggle Score:** 0.79186 (public leaderboard)

---

## Abstract

This project documents a complete machine learning classification pipeline developed for the Kaggle Titanic competition, which challenges competitors to predict passenger survival using demographic and ticket-level features. Beginning with a raw dataset of 891 training observations and 35 available features, the analysis proceeded through iterative feature engineering, multi-model training and cross-validated evaluation, and ensemble construction via hard voting across five classifiers (Logistic Regression, Gaussian Naive Bayes, Decision Tree, Random Forest, and Gradient Boosting). The best submission achieved 79.19% accuracy on the Kaggle public leaderboard — an improvement of 2.7 percentage points over the initial single-model baseline — placing the solution in approximately the top 22% of all competition submissions. The project demonstrates core applied ML skills directly transferable to people analytics use cases including attrition prediction, flight risk modeling, and candidate targeting.

---

## 1. Introduction

The Kaggle Titanic competition is one of the most widely used benchmarks in applied machine learning. Its enduring relevance stems from several characteristics that make it pedagogically rich: it is a binary classification problem (survived vs. did not survive), it contains a realistic mix of continuous and categorical features, it includes missing values requiring imputation decisions, and the ground truth is knowable — the historical record of who survived provides an external validation that purely simulated datasets lack.

Beyond its competition context, the Titanic problem models the core structure of a wide class of people analytics prediction challenges. Predicting voluntary attrition, identifying employees at risk of disengagement, or scoring candidates for outreach are all formally equivalent: each requires learning a binary outcome from a feature matrix of individual attributes, and each rewards the same skills — thoughtful feature engineering, principled model comparison, and ensemble construction.

This project was completed as part of a graduate Data Mining course at Seattle Pacific University (Winter 2022). The analysis iterated across four submission rounds, tracking public leaderboard performance at each step to demonstrate the incremental value of the ensemble approach.
...
Feature engineering — the construction of new predictive features from raw variables — produced larger accuracy gains than any single model choice or hyperparameter tuning decision. The following features were constructed:
...
The performance advantage of the voting ensemble over individual classifiers reflects a general principle in applied machine learning: when models make independent errors, aggregating their predictions reduces variance without increasing bias. The five classifiers used here span fundamentally different learning paradigms — linear (Logistic Regression), probabilistic (Naive Bayes), tree-based (Decision Tree, Random Forest), and sequential boosting (Gradient Boosting) — which promotes the diversity necessary for ensembles to outperform their components.

In people analytics applications, the same principle applies. An attrition prediction system that combines a logistic regression (for interpretability), a random forest (for non-linear interaction capture), and a gradient boosting model (for overall accuracy) is more robust and trustworthy than any single model — particularly important in high-stakes decisions affecting employees.

### 6.2 Feature Engineering vs. Model Tuning

The analysis confirms a common finding in applied ML competitions and production systems: the return on investment from feature engineering exceeds the return from model hyperparameter tuning. The transition from raw features to engineered features (Title extraction, family size, log-transformed fare, age imputation by subgroup) improved accuracy substantially, while marginal adjustments to hyperparameters produced modest gains.

This has direct implications for people analytics: the effort spent defining and constructing meaningful features (tenure bands, manager span of control, recent performance trajectory, internal mobility history) will generally outperform the effort spent selecting among algorithms.

### 6.3 Transfer to People Analytics

The Titanic classification pipeline is structurally identical to a voluntary attrition prediction model:

| Titanic | People Analytics Equivalent |
|:--------|:---------------------------|
| Survived (0/1) | Voluntary attrition in next 12 months (0/1) |
| Pclass | Pay grade / job level |
| Sex, Age | Demographic covariates |
| FamilySize | Team size / span of control |
| Title | Job family / role title |
| Embarked | Location / business unit |
| Fare | Compensation relative to band midpoint |

The ensemble architecture, stratified cross-validation workflow, and feature importance analysis demonstrated here are directly applicable to workforce prediction tasks.
...
This project demonstrates a complete supervised machine learning pipeline: raw data ingestion, iterative feature engineering, multi-model training, stratified cross-validated evaluation, and ensemble construction. The 5-model hard-voting ensemble achieved 79.19% accuracy on the Kaggle Titanic public leaderboard — an improvement of 2.7 percentage points over the single-model baseline. The feature engineering decisions (title extraction for age imputation, family size construction, fare log-transformation) contributed more to performance gains than model selection or hyperparameter tuning.

The same pipeline, adapted to workforce data, supports attrition prediction, flight risk identification, and other people analytics classification tasks where the goal is to identify binary outcomes at the individual employee level before they become organizationally costly.
