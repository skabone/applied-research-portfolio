...
The source dataset was retrieved from Kaggle. It was created by scraping job postings related to data science positions from Glassdoor using Selenium, capturing postings available during 2021. The public repository stores a feature-level analysis extract with 742 rows and 36 fields. Raw job descriptions, company names, headquarters, competitors, and contact-bearing posting text are intentionally excluded from this repository.
...
| Company information | company_rating, company_size, founded_year, company_age, ownership_type, industry, sector, revenue_band |
| Location | job_state |
| Tools/languages required | python, spark, aws, excel, sql, sas, keras, pytorch, scikit, tensor, hadoop, tableau, power_bi, flink, mongo, google_analytics |
| Other | role_family, seniority, degree_required |
...
The public extract keeps feature-level fields needed for the dashboard and excludes raw posting text or unnecessary third-party identifiers. Excluded fields include job descriptions, company names, headquarters, competitors, and any posting text that could carry contact details.

### 3.2 Column Renaming

Several columns were renamed for clarity and interpretability:
- `bi` to `power_bi`
- `Job Location` to `job_state`
- `google_an` to `google_analytics`

### 3.3 Binary Tool Column Conversion

All 16 tool/language columns were converted from integer indicators to boolean values. This ensures correct aggregation behavior in Power BI and prevents inadvertent summation of binary indicators.
...
| Market Segments | Role family, seniority, degree requirement, industry, and sector comparisons |
