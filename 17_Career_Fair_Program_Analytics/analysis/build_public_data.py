...
def _first_existing(base: Path, candidates: list[str]) -> Path:
...
def _public_event_name(value: object) -> str:
    label = str(value).strip()
    if "SPU" in label.upper():
        return "SPU Career Fair"
    return "Career Fair"
...
def build_public_tables(source_base: Path) -> dict[str, pd.DataFrame]:
    ...
    if "Career Fair" in reg_slim.columns:
        ...
        event_labels = reg_slim["Career Fair"].dropna().astype(str).unique()[:1].tolist()
        totals = {
            "career_fair": event_labels,
        ...
        out["event_totals"] = pd.DataFrame(
            [
                {
                    "career_fair": _public_event_name(totals["career_fair"][0]) if totals["career_fair"] else "",
